...
class Report():
    ...
    def addEmployee(self, fname):
        # Convert payroll items to string, hour and amount column to float
        df = pd.read_csv(fname)
        df.iloc[:, Table.PAYROLL_ITEM_COL] = df.iloc[:, Table.PAYROLL_ITEM_COL].str.lower().fillna('')
        df.iloc[:, Table.HOUR_COL] = df.iloc[:, Table.HOUR_COL].fillna(0).astype(float)
        df.iloc[:, Table.AMOUNT_COL] = df.iloc[:, Table.AMOUNT_COL].fillna(0).astype(float)
        
        # Process payroll items for each employee
        i = 0
        while(i < len(df) - 1):
